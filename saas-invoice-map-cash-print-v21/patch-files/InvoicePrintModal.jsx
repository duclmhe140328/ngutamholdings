...
const InvoicePrintModal = ({ order, shop, onClose, onSave }) => {
  ...
  const buildPrintHtml = (paper = 'a4') => {
    ...
    const pinnedText = fulfill.pinned ? `${fulfill.pinned.lat}, ${fulfill.pinned.lng}` : '';
    const mapButton = fulfill.mapUrl ? `<p><b>Google Maps:</b> <a href="${escapeHtml(fulfill.mapUrl)}" target="_blank" rel="noreferrer">${escapeHtml(fulfill.mapUrl)}</a></p>` : '';
    ...
    <style>
      *{box-sizing:border-box}body{margin:0;background:#ece8e1;color:#15120e;font:14px/1.45 Arial,sans-serif}.sheet{width:${isThermal ? '80mm' : '210mm'};min-height:${isThermal ? 'auto' : '297mm'};margin:16px auto;background:#fff;padding:${isThermal ? '6mm 4mm' : '14mm 15mm'};box-shadow:0 12px 38px rgba(0,0,0,.14)}
      ...
      <section class="info-grid">
        <div class="box"><h3>Thông tin người mua</h3><p><b>Người gọi món:</b> ${escapeHtml(customerNames)}</p><p><b>SĐT:</b> ${escapeHtml(order.loyaltyPhone || fulfill.phone || 'Không sử dụng')}</p><p><b>Đơn vị:</b> ${escapeHtml(form.buyerCompanyName || '—')}</p><p><b>MST:</b> ${escapeHtml(form.buyerTaxCode || '—')}</p><p><b>Địa chỉ:</b> ${escapeHtml(form.buyerAddress || fulfill.address || '—')}</p><p><b>Email:</b> ${escapeHtml(form.buyerEmail || '—')}</p></div>
        <div class="box"><h3>Thông tin giao dịch</h3><p><b>Loại đơn:</b> ${escapeHtml(fulfill.label)}</p><p><b>Phương thức:</b> ${escapeHtml(order.paymentMethod === 'multiple' ? 'Nhiều phương thức' : order.paymentMethod || '—')}</p><p><b>Thanh toán:</b> ${escapeHtml(order.paymentStatus === 'paid' ? `Đã thanh toán lúc ${dateTime(order.paidAt)}` : 'Chưa thanh toán / thu tiền mặt khi nhận hàng')}</p>${paymentHistory ? `<div class="payment-history"><b>Lịch sử thanh toán</b>${paymentHistory}</div>` : ''}<p><b>Ghi chú:</b> ${escapeHtml(fulfill.note || form.invoiceNote || '—')}</p></div>
      </section>
      ${(fulfill.isDelivery || fulfill.isPickup || fulfill.address || fulfill.note || fulfill.pinned) ? `<section class="box fulfillment"><h3>Thông tin nhận / giao hàng</h3><p><b>Hình thức:</b> ${escapeHtml(fulfill.label)}</p><p><b>Người nhận:</b> ${escapeHtml(fulfill.receiver || customerNames || '—')}</p><p><b>SĐT:</b> ${escapeHtml(fulfill.phone || '—')}</p>${fulfill.isPickup || fulfill.pickupTime ? `<p><b>Thời gian khách đến lấy:</b> ${escapeHtml(dateTime(fulfill.pickupTime))}</p>` : ''}${fulfill.address ? `<p><b>Địa chỉ:</b> ${escapeHtml(fulfill.address)}</p>` : ''}${fulfill.note ? `<p><b>Ghi chú khách:</b> ${escapeHtml(fulfill.note)}</p>` : ''}${pinnedText ? `<p><b>Tọa độ khách ghim:</b> ${escapeHtml(pinnedText)}</p>` : '<p class="map-note"><b>Tọa độ khách ghim:</b> Chưa thấy trong dữ liệu đơn. Cần kiểm tra route checkout đã lưu lat/lng vào order chưa.</p>'}${mapButton}</section>` : ''}
